...
mentor_groups = {
    'Harry(x) Plotter': [],
    'Gaussip Girls': [],
    'Fast and the Fouriers': [],
    'Alice i Underrummet': [],
    'SI-th units': [],
    'Quantum Tarantino': [],
    'Shrekvationssystemet': [],
    'Bumπbjörnarna': [],
    'Blixten Maclaurin': [],
    'ππ-långstrump': [],
    'Knatte Fnatte och Matte': [],
    'πngu1': [],
    'Funktionstein': [],
    'Stig Hilberts SÄLGskapsresa': [],
    'Hookes Bookus': []
}
...
prioritized_posts = []
if 'Ledningen' in prioritizations:
    prioritized_posts.extend(management)
if 'Föset' in prioritizations:
    prioritized_posts.extend(foset)
if 'Sångförmän' in prioritizations:
    prioritized_posts.append('Sångförman')
...
if type_of_event == 'Mentor group':
    ...
    raise ValueError('Expexted Mentor group or Mission group as type of event')

fileName = 'local/anmalda_till_' + event_name + '.csv'
fileName = fileName.replace(' ', '-')

sys.stdin.reconfigure(encoding='utf-8')
sys.stdout.reconfigure(encoding='utf-8')

with open('local/user_dic.json', 'r', encoding='UTF-8-SIG') as f:
    group_dictionary = json.load(f)

with open('local/post_dic.json', 'r', encoding='UTF-8-SIG') as f:
...
with open(fileName, 'r', encoding='UTF-8', newline='') as f:
    ...
    for line in f:
        ...
        if alcohol_package == 'Alkohol':
            price_package = price_alcohol
        elif alcohol_package == 'Alkoholfritt':
            price_package = price_alcoholfree
        else:
            price_package = 0
        ...
        if mail in post_dictionary:
            ...
            if any(post in prioritized_posts for post in user_data['Posts']):
                if mail in group_dictionary:
                    for g in group_dictionary[mail]['Groups']:
                        if g in groups:
                            group = g
                            type_of_user = 'Fadder'
                            groups[group].append(f'{name}, {0}, {price_package}, {0 + price_package}')
                            break
                ...
                prioritized_list.append([name, mail, foodpref, type_of_user, given_user_type, group, answer, '0', str(price_package), str(price_package), timestamp])
                nbr_of_spots += -1
                continue
        ...
        if mail in group_dictionary:
            ...
            if user_data['Type'] == 'mentor' and any(group in groups for group in user_data['Groups']):
                mentor_list.append(
                    [name, mail, foodpref, 'Fadder', given_user_type, group, answer, str(price_ticket), str(price_package), str(price_ticket+price_package), timestamp])

            elif user_data['Type'] == 'mentee':
                mentee_list.append(
                    [name, mail, foodpref, 'Nolla', given_user_type, group, answer, str(price_ticket), str(price_package), str(price_ticket+price_package), timestamp])

            else:
                other_list.append([name, mail, foodpref, 'Övrigt',
                                  given_user_type, group, answer, str(price_ticket), str(price_package), str(price_ticket+price_package), timestamp])
...
if nbr_of_spots < nbr_of_groups * nbr_of_guarantee_mentors:
    ...
    print(
        f'För få platser för att garantera {nbr_of_guarantee_mentors} faddrar i varje grupp. Ändrar till {new_nbr_of_guarantee_mentors } faddrar per grupp.')
...
while len(chosen_groups) < nbr_of_groups * nbr_of_guarantee_mentors:
    ...
    if len([i for i in chosen_groups if i == mentor_list[idx][5]]) < nbr_of_guarantee_mentors:
        ...
        groups[choice[5]].append(f'{choice[0]}, {price_ticket}, {price_package}, {price_ticket+price_package}')
        nbr_of_spots -= 1
...
with open('local/platsindelning.csv', 'w', encoding='UTF-8-SIG') as f:
    f.write('Namn, Mail, Matpreferens, Användartyp, Given användartyp, Grupp, Svar, Biljettpris, Pris dryckespaket, Totalpris, Anmälningstid \n')
    ...
    f.write('-------------------------------------')
    f.write('\n')
    f.write('antal nollor som inte fick plats: ' +
            str(len(mentee_list)) + '\n')
    f.write('-------------------------------------')
    f.write('\n')
    ...
    f.write('-------------------------------------')
    f.write('\n')
    f.write('antal faddrar som inte fick plats: ' +
            str(len(mentor_list)) + '\n')
    f.write('-------------------------------------')
    f.write('\n')
...
with open('local/betalinformation.csv', 'w', encoding='UTF-8-SIG') as f:

    for group in groups.keys():
        f.write('\n')
        f.write('\n')
        f.write(f'{group}: \n')
        f.write(f'Namn, Biljett, Dryckespaket, Totalt \n')

        total_ticket_price = 0
        total_alcohol_price = 0
        for person in groups[group]:
            f.write(f'{person} \n')
            person = person.split(',')
            total_ticket_price += int(person[1])
            total_alcohol_price += int(person[2])
        f.write(f'{len(groups[group])}, {total_ticket_price}, {total_alcohol_price}, {total_ticket_price+total_alcohol_price} \n')

    f.write('\n')
    f.write('\n')
    f.write(f'Anmodade och övriga: \n')
    f.write(f'Namn, Mail, Biljett, Dryckespaket, Totalt \n')
    for person in prioritized_list:
        if person[5] not in groups:
            f.write(f'{person[0]}, {person[1]}, {person[7]}, {person[8]}, {person[9]} \n')
